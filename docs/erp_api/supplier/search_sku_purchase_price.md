...
| sku_group | string | 否 | SKU分组，精确匹配 |
| sku_name | string | 否 | 商品名称，包含匹配 |
| sku | string | 否 | 商品SKU，包含匹配 |
...
1. 验证用户权限
2. 按当前项目ID隔离数据，并排除已软删除记录
3. 根据供应商名称、SKU分组、商品名称和商品SKU筛选记录
4. 对筛选后的结果进行总数统计和分页查询
5. 返回分页结果
...
- 接口会根据当前项目ID自动过滤数据
- 供应商名称、商品名称和商品SKU使用包含匹配，SKU分组使用精确匹配
- 筛选参数为空或仅包含空白字符时不参与过滤
