...
2. 按换行符切分 `skus` 字符串
3. 对每个 SKU：
   - 数据库存在有效同名记录 → 跳过（`ignored`）
   - 数据库存在已逻辑删除的同名记录 → 继续同步并通过 `is_delete=0` 恢复
...
- 已存在且有效的 SKU 不会重复添加，需要更新走 `save_sku`
- 已逻辑删除的同名 SKU 会刷新 ERP 信息并恢复
...
### 2026-07-19 - 支持恢复已删除 SKU

**变更类型**：已有记录判断语义调整。

**变更内容**：
- `is_delete` 为 `NULL` 或 `0` 的同名 SKU 计为 `ignored`
- 仅 `is_delete == 1` 的同名记录按新增流程刷新，并显式写入 `is_delete=0`
