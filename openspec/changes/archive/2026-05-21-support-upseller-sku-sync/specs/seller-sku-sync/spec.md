# seller-sku-sync 能力规约（增量）

本能力定义 ERP SKU 主数据/库存定时同步链路在统一 SellerClient 抽象下应满足的行为契约，覆盖 `auto_sync_tools/sync_all_sku.py`、`auto_sync_tools/sync_sku_inventory.py` 两个脚本。
